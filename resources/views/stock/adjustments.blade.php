...
@section('breadcrumb')
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <li class="flex items-center"><i class="fa-solid fa-chevron-right text-xs mx-2"></i> Stock Adjustments</li>
        </ol>
    </nav>
@endsection
...
<div x-data="adjustmentList()">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100">Stock Adjustments</h2>
        <button @click="openAddModal()" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-700">
            <i class="fa-solid fa-plus mr-2"></i> New Adjustment
        </button>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
        <div class="p-4 flex items-end gap-4">
            <div class="flex-1">
                ...
                <input type="date" x-model="filters.date" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm px-3 py-2 focus:ring-primary-500 focus:border-primary-500">
            </div>
            <div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $adj->date }}</td>
                        <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $adj->location->name ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ ($adj->type ?? '') === 'abnormal' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' }}">
                                {{ ucfirst($adj->type ?? 'normal') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 dark:text-gray-400 max-w-[200px] truncate">{{ $adj->notes }}</td>
                        <td class="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{{ $adj->items_count ?? 0 }}</td>
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('stock.adjustments.show', $adj->id ?? 0) }}" class="text-primary-600 hover:text-primary-800"><i class="fa-solid fa-eye"></i></a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">No stock adjustments found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100">New Stock Adjustment</h3>
                ...
                <form action="{{ route('stock.adjustments.store') }}" method="POST">
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="location_id" x-model="form.location_id" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm px-3 py-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="adjustment_type" x-model="form.adjustment_type" required class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm px-3 py-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                            ...
                            <div class="flex gap-2 mb-3">
                                ...
                                <div class="w-32">
                                    <input type="number" x-model="itemQty" placeholder="Qty" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm px-3 py-2 focus:ring-primary-500 focus:border-primary-500">
                                </div>
                                    ...
                                    <template x-for="(item, index) in items" :key="index">
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right">
                                                <input type="number" :name="'items[' + index + '][quantity]'" x-model="item.quantity" class="w-20 text-right rounded border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 text-sm px-2 py-1 focus:ring-primary-500 focus:border-primary-500">
                                            </td>
                                            <td class="px-3 py-2 text-right">
                    ...
                    <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit" class="px-6 py-2 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-700">Save Adjustment</button>
                    </div>
                </form>
            </div>
        </div>
