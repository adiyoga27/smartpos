...
@section('breadcrumb')
    <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
        ...
        <span class="text-gray-700 dark:text-gray-200">Edit - {{ $product->name }}</span>
    </nav>
@endsection

@section('content')
<div x-data="{ type: '{{ old('type', $product->type) }}', enableStock: {{ old('enable_stock', $product->enable_stock) ? 'true' : 'false' }}, purchasePrice: {{ old('purchase_price', $defaultVariation->default_purchase_price ?? 0) }}, profitPercent: {{ old('profit_percent', $defaultVariation->profit_percent ?? 0) }} }">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100">Edit Produk</h2>
    ...
    <form method="POST" action="{{ route('products.update', $product->id) }}" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $product->name) }}" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('name') border-danger-500 @enderror">
            ...
            <div>
                ...
                <select name="type" id="type" x-model="type" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('type') border-danger-500 @enderror">
                    <option value="single" {{ old('type', $product->type) == 'single' ? 'selected' : '' }}>Produk Satuan (Single)</option>
                    <option value="variable" {{ old('type', $product->type) == 'variable' ? 'selected' : '' }}>Produk Variasi (Variable)</option>
                </select>
            ...
            <div>
                ...
                <select name="unit_id" id="unit_id" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('unit_id') border-danger-500 @enderror">
                    ...
                    @foreach($units ?? [] as $unit)
                        <option value="{{ $unit->id }}" {{ old('unit_id', $product->unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->actual_name }} ({{ $unit->short_name }})</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="tax_id" id="tax_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('tax_id') border-danger-500 @enderror">
                    ...
                    @foreach($taxRates ?? [] as $tax)
                        <option value="{{ $tax->id }}" {{ old('tax_id', $product->tax_id) == $tax->id ? 'selected' : '' }}>{{ $tax->name }} ({{ $tax->amount }}%)</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <div class="flex gap-4 mt-2">
                    <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                        <input type="radio" name="tax_type" value="inclusive" {{ old('tax_type', $product->tax_type) == 'inclusive' ? 'checked' : '' }} class="text-primary-600 focus:ring-primary-500">
                        Inklusif (Termasuk)
                    </label>
                    <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                        <input type="radio" name="tax_type" value="exclusive" {{ old('tax_type', $product->tax_type) == 'exclusive' ? 'checked' : '' }} class="text-primary-600 focus:ring-primary-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            <div class="flex items-start gap-3">
                <label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer pt-1">
                    <input type="checkbox" name="enable_stock" value="1" x-model="enableStock" {{ old('enable_stock', $product->enable_stock) ? 'checked' : '' }} class="rounded text-primary-600 focus:ring-primary-500">
            ...
            <div x-show="enableStock">
                ...
                <input type="number" name="alert_quantity" id="alert_quantity" value="{{ old('alert_quantity', $product->alert_quantity ?? 0) }}" min="0" step="1" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('alert_quantity') border-danger-500 @enderror">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            <div>
                ...
                <input type="number" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $defaultVariation->default_purchase_price ?? 0) }}" step="0.01" min="0" x-model="purchasePrice" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('purchase_price') border-danger-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="profit_percent" id="profit_percent" value="{{ old('profit_percent', $defaultVariation->profit_percent ?? 0) }}" step="0.01" min="0" x-model="profitPercent" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('profit_percent') border-danger-500 @enderror">
            ...
            <div>
                ...
                <input type="number" name="sell_price" id="sell_price" value="{{ old('sell_price', $defaultVariation->default_sell_price ?? 0) }}" step="0.01" min="0" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('sell_price') border-danger-500 @enderror">
        ...
        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <textarea name="product_description" id="product_description" rows="4" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none @error('product_description') border-danger-500 @enderror">{{ old('product_description', $product->product_description) }}</textarea>
