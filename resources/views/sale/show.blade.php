...
@section('breadcrumb')
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <li><a href="{{ route('sales.index') }}" class="hover:text-primary-600">Penjualan</a></li>
            <li><span class="mx-1">/</span></li>
            <li class="text-gray-700 dark:text-gray-200 font-medium">#{{ $sale->invoice_no }}</li>
        </ol>
    </nav>
@endsection
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h2 class="text-lg font-bold text-gray-800 dark:text-gray-100">{{ $sale->location?->name ?? 'SmartPOS' }}</h2>
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $sale->location?->address }}</p>
            </div>
            <div class="text-right">
                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Invoice</p>
                <p class="text-lg font-bold text-gray-800 dark:text-gray-100">{{ $sale->invoice_no }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $sale->transaction_date?->format('d M Y, H:i') }}</p>
            </div>
            <div>
                <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Pelanggan</p>
                <p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $sale->contact?->full_name ?? 'Umum' }}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $sale->location?->name ?? '-' }}</p>
            </div>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400 uppercase text-xs tracking-wider">
                    <tr>
                        ...
                        <th class="text-right px-4 py-3 w-32">Harga Satuan</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($sale->items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-gray-300">Rp {{ number_format($item->unit_price, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right font-semibold text-gray-800 dark:text-gray-200">Rp {{ number_format($item->quantity * $item->unit_price, 0, ',', '.') }}</td>
                        </tr>
                    @empty
                        <tr>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-4">
        <div class="space-y-2">
            @php
                $saleSubtotal = $sale->items->sum(fn($i) => $i->quantity * $i->unit_price);
            @endphp
            <div class="flex justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">Subtotal</span>
                <span class="font-medium text-gray-800 dark:text-gray-200">Rp {{ number_format($saleSubtotal, 0, ',', '.') }}</span>
            </div>
            @if($sale->discount_amount > 0)
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600 dark:text-gray-400">Diskon {{ $sale->discount_type === 'percentage' ? '('.$sale->discount_amount.'%)' : '' }}</span>
                    <span class="font-medium text-danger-600">-Rp {{ number_format($sale->discount_amount, 0, ',', '.') }}</span>
                </div>
            @endif
            @if($sale->tax_amount > 0)
                <div class="flex justify-between text-sm">
                    <span class="text-gray-600 dark:text-gray-400">Pajak {{ $sale->tax?->name ? '('.$sale->tax->name.')' : '' }}</span>
                    <span class="font-medium text-gray-800 dark:text-gray-200">Rp {{ number_format($sale->tax_amount, 0, ',', '.') }}</span>
                </div>
            @endif
            <div class="flex justify-between text-base font-bold border-t border-gray-200 dark:border-gray-600 pt-2 mt-2">
                ...
                <span class="text-primary-600 dark:text-primary-400">Rp {{ number_format($sale->final_total, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
                            ...
                            <tr>
                                <td class="px-4 py-3 text-gray-800 dark:text-gray-200">
                                    ...
                                    @elseif($payment->method === 'card')
                                        <i class="fa-solid fa-credit-card mr-1.5 text-primary-500"></i> Kartu
    ...
    <div class="mt-4 flex gap-2">
        <a href="{{ route('sales.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
