...
@section('breadcrumb')
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="flex items-center gap-1 text-sm">
            ...
            <li><span class="text-gray-900 dark:text-white font-medium">Merek</span></li>
        </ol>
    </nav>
@endsection

@section('content')
<div x-data="{ open: false, editMode: false, form: { id: null, name: '', description: '' } }">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Daftar Merek</h2>
            <button x-on:click="open = true; editMode = false; form = { id: null, name: '', description: '' }"
                class="inline-flex items-center px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors">
                <i class="fa-solid fa-plus mr-2"></i> Tambah Merek
            </button>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Deskripsi</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($brands ?? [] as $brand)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            <td class="px-6 py-4 text-sm text-gray-900 dark:text-white font-medium">{{ $brand->name }}</td>
                            <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">{{ $brand->description ?? '-' }}</td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button x-on:click="open = true; editMode = true; form = { id: {{ $brand->id }}, name: '{{ addslashes($brand->name) }}', description: '{{ addslashes($brand->description ?? '') }}' }"
                                    ...
                                    <form action="{{ route('brands.destroy', $brand->id) }}" method="POST" onsubmit="return confirm('Hapus merek ini?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-12 text-center text-gray-400 dark:text-gray-500">
                                ...
                                Belum ada merek. Klik "Tambah Merek" untuk memulai.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if(method_exists($brands ?? new \Illuminate\Database\Eloquent\Collection, 'links'))
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white" x-text="editMode ? 'Edit Merek' : 'Tambah Merek'"></h3>
                ...
                <form x-bind:action="editMode ? '{{ route('brands.index') }}/' + form.id : '{{ route('brands.store') }}'" method="POST" class="space-y-4">
                    @csrf
                    ...
                    <div>
                        <label for="brand_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Merek</label>
                        <input id="brand_name" name="name" type="text" x-model="form.name" required
                    ...
                    <div>
                        <label for="brand_description" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Deskripsi</label>
                        <textarea id="brand_description" name="description" rows="3" x-model="form.description"
                            class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm"
