...
@section('breadcrumb')
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 text-sm text-gray-500 dark:text-gray-400">
            ...
            <li class="text-gray-700 dark:text-gray-200 font-medium">Laporan Pembelian</li>
        </ol>
    </nav>
@endsection
...
<div>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">
            <i class="fa-solid fa-truck mr-2 text-primary-500"></i> Laporan Pembelian
        </h2>
    </div>
    ...
    <div class="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-gray-800 dark:text-gray-100">Rp {{ number_format($totalPurchases, 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4 mb-4">
        <form method="GET" class="flex flex-col sm:flex-row gap-3 items-end">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('reports.purchases') }}" class="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    Reset
                </a>
            </div>
        </form>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($purchases as $purchase)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                            ...
                            <td class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $purchase->contact?->full_name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                @elseif($purchase->status === 'ordered')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-warning-50 text-warning-600 dark:bg-warning-600/20">
                                        <i class="fa-solid fa-clock mr-1"></i> Dipesan
                                    </span>
                                @elseif($purchase->status === 'pending')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600 dark:bg-gray-600/20 dark:text-gray-400">
                                        <i class="fa-solid fa-hourglass-half mr-1"></i> Tertunda
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-danger-50 text-danger-600 dark:bg-danger-600/20">
                                        <i class="fa-solid fa-circle-exclamation mr-1"></i> {{ $purchase->status }}
                                    </span>
                                @endif
                            </td>
                        </tr>
